...
class Chirpify_Seller_Model_Service_Api_V2_Order
{
  ...
  {
    $campaign = $this->_initCampaign( $info );
    ...
    $info = $this->_registerInfo( $info, $quote_id, $store_id, $campaign );
    # add a product
    $cart_product_api = $this->_addProductToCart( $quote_id, $store_id, $campaign );
    # add customer
  ...
  protected function _addProductToCart( $quote_id, $store_id, $campaign )
  {
    $cart_product_api = Mage::getModel( 'checkout/cart_product_api' );
    $product_id = $this->_getProductIdFromCampaign( $campaign );
  ...
  protected function _getProductPriceFromCampaignAction( $campaign )
  {
    return $campaign->getPrice();
  }

  protected function _getProductIdFromCampaignAction( $campaign )
  {
    return $campaign->getProductId();
  }
  ...
  protected function _initCampaign( $info )
  {
    if( !$this->_campaign )
    {
      $this->_campaign = Mage::getModel( 'chirpify_seller/campaign' )->load( $info['campaign']['meta']['magento_campaign_id'] );
    }

    if( !$this->_campaign->isActive() )
    {
      Mage::log( "Campaign is not active" );
      Mage::throwException( 'Inactive Campaign/Campaign' );
    }
  ...
  protected function _registerInfo( $info, $quote_id, $store_id, $campaign )
  {
    ...
    $info['price']          = array_key_exists( 'price', $info['campaign'] ) ? $info['campaign']['price'] : $this->_getProductPriceFromCampaign( $campaign );
    $info['shipping_price'] = array_key_exists( 'shipping_price', $info['campaign'] ) ? $info['campaign']['shipping_price'] : $this->_getShippingProductPriceFromCampaign( $campaign );
  ...
  /*
   * need to manage order status bassed on digital vs physical campaign
   *
   */
